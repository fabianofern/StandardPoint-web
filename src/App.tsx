...
const useHashRouter = () => {
  ...
  const parseHash = useCallback((hash: string): HashState => {
...
function AppContent() {
  ...
  const handleNavigate = useCallback((page: string, navParams: any = {}) => {
      ...
      >
        ...
        <main style={{
          ...
          {currentPage === 'valor-ajuste' && <ValorAjusteContent
            ...
            onVAChange={(vafData: any) => saveVAF(projetoAtual!, vafData)}
          />}
...
function DashboardContent({ onNavigate }: DashboardContentProps) {
  const {
...
function MinhasEmpresasContent({ onNavigate }: DashboardContentProps) {
...
function EmpresaContent({ isCreatingNew, empresaId, onNavigate, onBackToDashboard, onBackToEmpresas }: EmpresaContentProps) {
