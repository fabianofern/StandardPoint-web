...
export const importBackupToDb = db.transaction((dadosBackup: any) => {
  ...
  const insertProjeto = db.prepare('INSERT INTO projetos (id, empresa_id, nome, descricao, data_inicio_contagem, data_fim_contagem, tipo_contagem, vaf) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
  ...
  for (const emp of empresas) {
    ...
    for (const proj of projetos) {
      insertProjeto.run(
        ...
        proj.tipoContagem || 'desenvolvimento',
        proj.vaf ? JSON.stringify(proj.vaf) : null
      );
...
export const exportDbToBackup = () => {
   const empresasRaw = db.prepare('SELECT * FROM empresas').all() as any[];
   
   const empresas = empresasRaw.map(emp => {
      const cargos = (db.prepare('SELECT * FROM cargos WHERE empresa_id = ?').all(emp.id) as any[]).map(c => ({
         id: c.id,
         titulo: c.titulo,
         fatorPF: c.fator_pf
      }));

      const funcionarios = (db.prepare('SELECT * FROM funcionarios WHERE empresa_id = ?').all(emp.id) as any[]).map(f => ({
         id: f.id,
         cargoId: f.cargo_id,
         nome: f.nome,
      ...
      const projetosRaw = db.prepare('SELECT * FROM projetos WHERE empresa_id = ?').all(emp.id) as any[];
      const projetos = projetosRaw.map(proj => {
         const squad = (db.prepare('SELECT * FROM squad_membros WHERE projeto_id = ?').all(proj.id) as any[]).map(sq => ({
            id: sq.id,
            funcionarioId: sq.funcionario_id
         }));

         const requisitos = (db.prepare('SELECT * FROM requisitos WHERE projeto_id = ?').all(proj.id) as any[]).map(req => ({
            id: req.id,
            descricao: req.descricao,
            tipo: req.tipo
         }));

         const funcoes = (db.prepare('SELECT * FROM funcoes WHERE projeto_id = ?').all(proj.id) as any[]).map(f => ({
            id: f.id,
         ...
         return {
            ...
            tipoContagem: proj.tipo_contagem,
            vaf: proj.vaf ? JSON.parse(proj.vaf) : null,
            squad,
            requisitos,
            funcoes
         };
